...
with st.sidebar:
    ...
    # KPI strip (Dynamic Specialist Version)
    _dvf_data = load_dvf()
    _n_trans = len(_dvf_data) if _dvf_data is not None else 123329
    _best_metrics = load_metrics()
    _best_mae = 1871 # Fallback
    if _best_metrics:
        _best_mae = min(m["MAE"] for m in _best_metrics)

    st.markdown(
        f'<div style="background:#131929;border:1px solid #1E2D45;border-radius:10px;padding:12px 14px;margin-bottom:12px">'
        f'<div style="font-size:0.7em;color:#8899BB;text-transform:uppercase;letter-spacing:1px;margin-bottom:6px">Fédération de Données V3 Pro</div>'
        f'<div style="color:#F0F4FF;font-weight:700;font-size:1em">{_n_trans:,} <span style="color:#8899BB;font-weight:400;font-size:0.8em">transactions</span></div>'
        f'<div style="color:#8899BB;font-size:0.75em;margin-top:1px">DVF Paris · 2021–2024 (Index 2025)</div>'
        f'<div style="margin-top:8px;border-top:1px solid #1E2D45;padding-top:8px">'
        f'<span style="color:#00D4AA;font-weight:700">MAE {_best_mae:,.0f} €/m²</span>'
        f' <span style="color:#8899BB;font-size:0.8em">· V3 High-Precision</span>'
        f'</div>'
        f'<div style="margin-top:6px">'
        f'<span style="background:linear-gradient(135deg,#00D4AA22,#0095FF22);border:1px solid #00D4AA55;'
        f'color:#00D4AA;border-radius:20px;padding:2px 10px;font-size:0.78em;font-weight:700">'
        f'💎 IA Spécialiste Active</span>'
        f'</div>'
        f'</div>',
        unsafe_allow_html=True,
    )
...
# ════════════════════════════════════════════════════════════════════
    ...
    if submitted:
                ...
                for gem in gems:
                    ...
                    with st.container():
                        ...
                        gc1.metric("Prix transaction", f"{gem.prix_total_transaction:,.0f} €",
                                   help="Prix DVF réel enregistré")
                        gc2.metric("Estimation Marché", f"{gem.prix_total_min:,.0f} - {gem.prix_total_max:,.0f} €",
                                   help="Fourchette de réalité basée sur l'état du bien et la volatilité du secteur")
        ...
        st.info(
            "**Comment ça marche ?**\n\n"
            "1. Le modèle ML (V3 Pro) prédit le prix/m² sur 4 ans de données (2021-2024).\n"
            "2. Toute transaction où `prix_prédit > prix_réel × 1.10` est identifiée comme *Hidden Gem*.\n"
            "3. L'IA applique un **indice de correction 2025** pour ajuster les prix historiques.\n"
            "4. Les résultats affichent une **fourchette de confiance (±10%)** pour refléter la réalité du marché.\n\n"
            "**Note :** Le modèle utilise la précision 'rue par rue' (Street-level Encoding) pour minimiser les écarts."
        )
